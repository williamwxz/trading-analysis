...
Mini trading analytics pipeline. Streams strategy PnL data: Binance API → streaming producer (Kafka/Redpanda) → pnl_consumer → ClickHouse Cloud analytics → Grafana Cloud dashboards. Batch recompute/repair is handled by `scripts/audit_pnl.py`; market-data gap-fill is a daily `backfill_prices` Lambda; scheduled `fix-window` Lambdas (one image, `services/fix_bt_window/`) repair the trailing PnL window per mode — bt daily (retro-corrects `strategy_cum_pnl_bt_v2` publish lag), prod and real_trade 6-hourly (retro-correct late-arriving revisions). Services run on AWS ECS Fargate in `ap-northeast-1` (Tokyo). Grafana is Grafana Cloud (not self-hosted).
...
# Full bt 1-min rebuild [GLOBAL_START_TS, now), monthly chunks (pause live bt consumer first)
caffeinate -dims python scripts/audit_pnl.py --rebuild-bt   # resumable via --rebuild-start
```
...
Batch PnL recompute/repair is the standalone script `scripts/audit_pnl.py` (not a service). Market-data historical gap-fill (`futures_price_1min`) is `services/backfill_prices/` — a daily AWS Lambda using ccxt. `services/fix_bt_window/` is a mode-parameterized AWS Lambda image (env `AUDIT_TYPE` / `CONSUMER_SERVICE` / `LOOKBACK_HOURS`) deployed as three scheduled functions that each pause their consumer, run `audit_pnl --type <mode> --fix-window` over the trailing window, and resume it (the post-resume bootstrap re-seeds the consumer's chain from the repaired tail): `fix-bt-window` daily 15:00 UTC over 49h (late `strategy_cum_pnl_bt_v2` batches — the cum publisher delivers hours-to-days after bar time), `fix-prod-window` and `fix-real-trade-window` 6-hourly (03:10/03:40 + every 6h) over 7h (first revisions land p50 ~16 min / p99 ~96 min past bar close, so live-written rows hold stale positions until repaired).
...
fix-window Lambdas (one image, three schedules) — pause mode's consumer →
    audit_pnl --type <mode> --fix-window [now-Lh, now) → resume
    bt daily 15:00 UTC, L=49h  — retro-corrects cum-table publish lag
    prod + real_trade 6-hourly, L=7h — retro-correct late first revisions
...
- `--rebuild-bt` rebuilds the entire bt 1-min table over `[GLOBAL_START_TS, now)` in calendar-month chunks (oldest→newest), forward-seeding each chunk from the prior chunk's tail; resumable via `--rebuild-start TS`. Pause the live bt consumer first (it does not auto-pause). Run under `caffeinate`.
...
1. **Fetch anchors**: `fetch_anchors()` reads the last committed row per strategy from the target table just before `start` (previous day's tail)
2. **Delete window**: idempotent DELETE WHERE ts >= start AND underlying = X
...
Prod uses `argMin(row_json, revision_ts)` (first revision only) with `iter_compute_prod_pnl`. BT minute-chains via `compute_bt_pnl` (batch) / `AnchorState` (live), sourcing per-bar `pos_first` + cold-start `cum_pnl_first` from `strategy_cum_pnl_bt_v2` and reading only the per-bar benchmark from `strategy_output_history_bt_v2` (`argMin(row_json,'benchmark')`).
...
**Cold-start bootstrap (prod / real_trade):**
...
**Cold-start bootstrap (bt)** — `_bootstrap_bt_state`, simpler: seed `(cumulative_pnl, price)` **plus bar metadata** per strategy from the `strategy_pnl_1min_bt_v2` tail before `reference_ts` (`fetch_last_pnl_anchors` returns the stored row's metadata columns; a metadata-less record is silently skipped by carry-forward — incident 2026-07-13). No history-shaped position seed and no walk-verify (the cum table is authoritative); position is supplied per-candle from the cum table. Brand-new strategies lazy-seed `cum_pnl_first` (price=0 hold) on first appearance.

**Carry-forward metadata invariant (all modes):** carry-forward rows require `underlying` + `strategy_instance_id` on the `AnchorRecord`. Every bootstrap path must seed them (prod/rt: from seeds, or from the stored anchor row for retired/no-seed strategies; bt: from the stored anchor row). The prod/rt candle-lookup bar-ts lower bound is floored to midnight (`toStartOfDay(candle_ts - 2 DAY)`) so the newest arrived 1d bar stays lookup-visible through the just-after-midnight window (new 1d bars only land ~00:31; without the floor every 1d strategy vanished from the lookups for the first ~half hour of each day, incident 2026-07-13 00:25/00:26).

**Live loop — prod:**
- Re-query `strategy_output_history_v2` every candle; latest bar whose `closing_ts <= candle_ts`, first revision only (`argMin`)
