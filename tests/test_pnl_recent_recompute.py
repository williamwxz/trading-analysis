...
class TestProcessUnderlyingRecent:
    ...
    def test_anchors_loaded_before_delete(self, mock_gc, mock_resume, mock_fa, mock_exec, mock_qd, mock_prices, mock_insert):
    ...
    @patch("trading_dagster.assets.pnl_strategy_v2._get_underlying_resume_dt", return_value=None)
    @patch("trading_dagster.assets.pnl_strategy_v2.get_client")
    def test_delete_uses_correct_table_and_window(self, mock_gc, mock_resume, mock_fa, mock_exec, mock_qd, mock_prices, mock_insert):
    ...
    @patch("trading_dagster.assets.pnl_strategy_v2._get_underlying_resume_dt", return_value=None)
    @patch("trading_dagster.assets.pnl_strategy_v2.get_client")
    def test_recompute_only_covers_window(self, mock_gc, mock_resume, mock_fa, mock_exec, mock_qd, mock_prices, mock_insert):
    ...
    @patch("trading_dagster.assets.pnl_strategy_v2.fetch_anchors")
    @patch("trading_dagster.assets.pnl_strategy_v2.get_client")
    def test_uses_resume_dt_when_data_exists(self, mock_gc, mock_fa, mock_exec, mock_qd, mock_prices, mock_insert):
        ...
        default_window_start = datetime(2026, 2, 27, 0, 0, 0, tzinfo=UTC)  # far back
        ...
        delete_sql = mock_exec.call_args[0][0]
        assert "2026-05-08 00:00:00" in delete_sql
        assert "2026-02-27" not in delete_sql  # default was NOT used
    ...
    @patch("trading_dagster.assets.pnl_strategy_v2._get_underlying_resume_dt", return_value=None)
    @patch("trading_dagster.assets.pnl_strategy_v2.get_client")
    def test_uses_default_window_when_no_data(self, mock_gc, mock_resume, mock_fa, mock_exec, mock_qd, mock_prices, mock_insert):
        ...
        default_window_start = datetime(2026, 2, 27, 0, 0, 0, tzinfo=UTC)
        end_dt = datetime(2026, 5, 11, 0, 0, 0, tzinfo=UTC)
        _process_underlying_recent(
            "btc", "strategy_pnl_1min_prod_v2", "strategy_output_history_v2",
            "production", PROD_INSERT_COLUMNS, "prod", default_window_start, end_dt,
...
class TestRecomputePnlRecent:
    ...
    def test_default_window_start_is_prod_real_trade_start_date(
        ...
        # _get_underlying_resume_dt returned None → _process_underlying_recent uses default_window_start
        # which equals PROD_REAL_TRADE_START_DATE; verify DELETE used that date
        delete_sql = mock_exec.call_args[0][0]
        assert PROD_REAL_TRADE_START_DATE in delete_sql  # e.g. "2026-02-27"
