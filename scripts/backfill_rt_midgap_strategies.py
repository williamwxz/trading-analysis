"""Backfill real_trade PnL rows for strategies with mid-history gaps.

Detects strategies that have RT rows both before AND after some date range
but are completely missing rows during that range.  Detection: a strategy's
count of distinct covered days is less than the number of calendar days
between its first and last RT row (i.e. it has holes in its daily coverage).

Start-gap strategies (absent entirely or missing early history) are handled
by backfill_rt_missing_strategies.py — this script ONLY handles mid-history gaps.

For each affected strategy: DELETE all its existing RT rows, then recompute
full history from scratch using build_rt_lookup + active_rt_revision_at.

Pauses trading-analysis-pnl-consumer-real-trade before writing, resumes after.

Usage:
    python scripts/backfill_rt_midgap_strategies.py [--dry-run]
"""
...
def find_midgap_strategies(client) -> dict[str, list[str]]:
    ...
    A strategy has a mid-history gap if the count of distinct days it has RT
    rows is less than the number of calendar days between its first and last
    RT row.  This means at least one day in its RT coverage window has zero rows.

    Strategies that are absent entirely or whose gaps are only at the start
    are NOT returned here — those belong to backfill_rt_missing_strategies.py.
    """
    sql = """
    SELECT
        strategy_table_name,
...
def main():
    ...
    gap_strategies = find_midgap_strategies(client)
    if not gap_strategies:
        log.info("No strategies with mid-history gaps found — nothing to do")
        return

    total_stns = sum(len(v) for v in gap_strategies.values())
    log.info(
        f"Found {total_stns} strategies with mid-history gaps "
        f"across {len(gap_strategies)} underlyings"
    )
