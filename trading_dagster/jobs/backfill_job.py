...
Manually-triggered job that runs a full daily backfill:
  binance_futures_backfill → pnl_prod_v2_daily + pnl_real_trade_v2_daily

pnl_1hour_rollup is excluded: it is unpartitioned and mixing it with
partitioned assets breaks the Dagster UI launchpad. Run it separately
